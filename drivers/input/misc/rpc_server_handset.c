...
#include <linux/of.h>
#include <soc/qcom/msm_rpcrouter.h>
...
enum hs_ext_cmd_type {
	...
	HS_EXT_CMD_DIAG_KEYMAP,			/* Emulating a Diag key
						   sequence */
	HS_EXT_CMD_DIAG_LOCK,			/* Device Lock/Unlock */
	HS_EXT_CMD_GET_EVNT_STATUS,		/* Get the status for one of the
						   drivers */
	HS_EXT_CMD_KPD_GET_KEYS_STATUS,		/* Get a list of keys status */
	HS_EXT_CMD_KPD_SET_PWR_KEY_RST_THOLD,	/* PWR Key HW Reset duration */
	HS_EXT_CMD_KPD_SET_PWR_KEY_THOLD,	/* Set pwr key threshold
						   duration */
	HS_EXT_CMD_LAST,			/* Should always be the last
						   command type */
	HS_EXT_CMD_MAX = 0x7FFFFFFF		/* Force enum to be an 32-bit
						   number */
};
...
struct hs_cmd_data_type {
	...
	uint32_t handle;		/* handle returned from subscribe
					   proc */
...
static struct msm_handset *hs_data;
...
static void report_hs_key(uint32_t key_code, uint32_t key_parm)
{
	...
	case SW_HEADPHONE_INSERT_W_MIC:
		...
		input_report_switch(hs->ipdev, SW_MICROPHONE_INSERT,
				    hs->mic_on);
		extcon_set_cable_state_(hs->edev, EXTCON_JACK_HEADPHONE,
					hs->mic_on);
		break;
	...
	case SW_MICROPHONE_INSERT:
		...
		extcon_set_cable_state_(hs->edev, EXTCON_JACK_MICROPHONE,
					hs->mic_on);
		break;
	case -1:
		pr_err("%s: No mapping for remote handset event %d\n",
			      ...
			      struct rpc_request_hdr *req, unsigned len)
{
	...
	case RPC_KEYPAD_SET_PWR_KEY_STATE_PROC:
		/*
		 * This RPC function must be available for the ARM9 to function
		 * properly. This function is redundant when
		 * RPC_KEYPAD_PASS_KEY_CODE_PROC is handled. So input_report_key
		 * is not needed.
		 */
		return 0;
	default:
			      ...
			      void *buffer, void *data)
{
	...
	hs_pwr_cmd->input_val	= cpu_to_be32(hs->pwr_key_delay_ms);
...
static int hs_cb_func(struct msm_rpc_client *client, void *buffer, int in_size)
{
	...
	process_hs_rpc_request(hdr->procedure, (void *) (hdr + 1));

	msm_rpc_start_accepted_reply(client, hdr->xid, RPC_ACCEPTSTAT_SUCCESS);
...
{
	int ret = 0, i, num_vers;

	num_vers = ARRAY_SIZE(rpc_vers);

	for (i = 0; i < num_vers; i++) {
		rpc_client = msm_rpc_register_client("hs", HS_RPC_PROG,
						     rpc_vers[i], 0,
						     hs_cb_func);
	...
	if (IS_ERR(rpc_client)) {
		ret = PTR_ERR(rpc_client);
		pr_err("%s: Incompatible RPC version error %d\n",
		       __func__, ret);
		return ret;
	}

	ret = msm_rpc_client_req(rpc_client, HS_SUBSCRIBE_SRVC_PROC,
	...
	ret = msm_rpc_client_req(rpc_client, HS_PROCESS_CMD_PROC,
...
static const unsigned int hs_extcon_cable[] = {
...
{
	struct msm_handset *hs;
	struct device *dev = &pdev->dev;
	struct device_node *node = dev->of_node;
	struct input_dev *ipdev;
	int ret = 0;

	hs = devm_kzalloc(dev, sizeof(struct msm_handset), GFP_KERNEL);
	if (!hs)
		return -ENOMEM;

	hs_data = hs;

	of_property_read_u32(node, "pwr-key-delay-ms", &hs->pwr_key_delay_ms);

	hs->edev = devm_extcon_dev_allocate(dev, hs_extcon_cable);
	if (IS_ERR(hs->edev)) {
		ret = PTR_ERR(hs->edev);
		dev_err(dev, "failed to allocate extcon ret=%d\n", ret);
		return ret;
	}

	ret = extcon_dev_register(hs->edev);
	if (ret)
		return ret;
	...
	if (ret) {
		dev_err(dev, "failed to register input device ret=%d\n", ret);
		goto err_reg_input_dev;
	}

	ret = hs_rpc_init();
	if (ret) {
		dev_err(dev, "rpc init failure ret=%d\n", ret);
...
static struct of_device_id hs_of_match_table[] = {
	{ .compatible = "qcom,msm-handset", },
	{ /* end of table */ }
};
MODULE_DEVICE_TABLE(of, hs_of_match_table);
...
static struct platform_driver hs_driver = {
	...
	.driver	= {
		.name		= "msm-handset",
		.owner		= THIS_MODULE,
		.of_match_table	= hs_of_match_table,
	},
};
