...
static int __axi_free(int mode)
{
	...
	if (rc) {
		pr_err("%s: AXI bus device (%d) failed to be configured\n",
		       __func__, rc);
		goto fail_dal_fcn_0;
	}
	...
	if (rc) {
		pr_err("%s: failed to detach AXI bus device (%d)\n",
		       __func__, rc);
...
fail_dal_fcn_0:
	daldevice_detach(cam_dev_handle);
	cam_dev_handle = NULL;
...
static int __axi_allocate(int mode)
{
	...
	if (rc) {
		pr_err("%s: failed to attach AXI bus device (%d)\n",
		       __func__, rc);
		goto fail_dal_attach_detach;
	}
...
static int axi_configure_bridge_grfx_sync_mode(int bridge_mode)
{
	...
	rc = dalrpc_fcn_0(DALRPC_AXI_CONFIGURE_BRIDGE, dev_handle, bridge_mode);
	if (rc) {
		pr_err("%s: AXI bus device (%d) failed to be configured\n",
		       __func__, rc);
		goto fail_dal_fcn_0;
