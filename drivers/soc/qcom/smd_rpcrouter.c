...
static int msm_rpc_reboot_call(struct notifier_block *this, unsigned long code,
			       void *_cmd)
{
...
static void modem_reset_cleanup(struct rpcrouter_xprt_info *xprt_info)
{
	...
	list_for_each_entry(ept, &local_endpoints, list) {
		...
		list_for_each_entry_safe(pkt, tmp_pkt, &ept->incomplete, list) {
			list_del(&pkt->list);
			frag = pkt->first;
			while (frag != NULL) {
				next = frag->next;
		...
		list_for_each_entry_safe(pkt, tmp_pkt, &ept->read_q, list) {
			list_del(&pkt->list);
			frag = pkt->first;
			while (frag != NULL) {
				next = frag->next;
				  ...
				  uint32_t prog, uint32_t vers)
{
	...
	if (r_ept && (r_ept->quota_restart_state != RESTART_NORMAL)) {
...
static void *rr_malloc(unsigned sz)
{
	...
	pr_err("%s: kmalloc of %d failed, retrying...\n", __func__, sz);
	do {
...
static void do_read_data(struct work_struct *work)
{
	...
	struct rpcrouter_xprt_info *xprt_info =
		container_of(work, struct rpcrouter_xprt_info, read_data);
...
static void debugfs_init(void)
{
	...
	debug_create("dump_servers", 0444, dent, dump_servers);
