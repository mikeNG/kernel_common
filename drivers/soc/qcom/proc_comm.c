...
static int proccomm_probe(struct platform_device *pdev)
{
	...
	res = platform_get_resource_byname(pdev, IORESOURCE_MEM, "shared_ram");
	data->shared_ram = devm_ioremap_resource(dev, res);
	if (IS_ERR(data->shared_ram)) {
		ret = PTR_ERR(data->shared_ram);
		dev_err(dev, "failed to ioremap 'shared_ram' ret=%d\n", ret);
		return ret;
	}
