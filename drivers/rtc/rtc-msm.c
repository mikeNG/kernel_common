...
static int msmrtc_timeremote_set_time_secure(struct device *dev,
					     struct rtc_time *tm)
{
	int rc;
	struct rtc_tod_args rtc_args;
	...
	rc = msm_rpc_client_req(rtc_pdata->rpc_client,
				...
				NULL, NULL, -1);
	if (rc) {
		dev_err(dev, "%s: rtc secure time could not be set\n",
			__func__);
		return rc;
	}
...
static int msmrtc_timeremote_read_time_secure(struct device *dev, struct rtc_time *tm)
{
	int rc;
	struct rtc_tod_args rtc_args;
	struct msm_rtc *rtc_pdata = dev_get_drvdata(dev);
	...
	if (rc) {
		dev_err(dev, "%s: Error retrieving secure rtc time\n",
			__func__);
		return rc;
	}
...
static void process_cb_request(void *buffer)
{
	...
	if (rtc_cb->event == EVENT_TOD_CHANGE) {
		...
		pr_info("TOD UPDATE (stubbed): ttick=%d stamp=%lld, freq=%d\n",
			rtc_cb->cb_info_data.tod_update.tick,
			rtc_cb->cb_info_data.tod_update.stamp,
			rtc_cb->cb_info_data.tod_update.freq);
	} else {
...
static int msmrtc_probe(struct platform_device *pdev)
{
	...
	rtc_pdata = devm_kzalloc(&pdev->dev, sizeof(*rtc_pdata), GFP_KERNEL);
	if (!rtc_pdata) {
	...
	if (IS_ERR(rtc_pdata->rpc_client)) {
		dev_err(&pdev->dev, "%s: init RPC failed! VERS = %x\n",
			__func__, prog_version);
...
MODULE_DESCRIPTION("RTC driver for Qualcomm MSM chipsets");
